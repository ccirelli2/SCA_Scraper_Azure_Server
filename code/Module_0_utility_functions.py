...
def get_num_pages_scraped(mydb, Beginning_page, table):
    '''
    ...
    sql_command = ''' 
        SELECT  COUNT(*)
        FROM {} 
        WHERE SUBSTRING_INDEX(page_number, '=', -1) > {}'''.format(table, Beginning_page)   
    mycursor.execute(sql_command)
    for x in mycursor:
        return x['COUNT(*)']   


def get_df_data_companies_sued(mydb, Beginning_page, table):
...
def driver_function_post_run_scraper_report(mydb, Beginning_page, End_page, report_output, 
    ...
    if report_output == 'print_results':
        df_companies_sued = get_df_data_companies_sued(mydb, Beginning_page, table)
